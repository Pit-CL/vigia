...
const SHELL_CACHE = 'vigia-shell-v35';
...
self.addEventListener('fetch', (e) => {
  ...
  const isData = isOpenMeteo ||
    /\/(status|verificacion|estaciones|aire|bias|avisos|sismos|incendios|alertas|volcanes|emergencia|remociones|tsunami_vias|tsunami_areas|marea|tsunami|comunas|cortes|farmacias)\.json$/.test(url.pathname);
  if (isData) {
    e.respondWith(
      fetch(e.request)
        .then((res) => {
